...
const tutorTurnSchema = z.object({
  learnerId: z.string().min(1), name: z.string().min(1).optional(), proficiency: proficiency.optional(), firstLanguage: z.string().min(2).optional(),
  skill: tutorSkill, learningGoal: z.string().min(1), message: z.string().max(12000).default(''),
...
function buildTutorMessage(input: TutorTurnInput, action: string, specialist: Record<string, unknown> | null) {
  ...
  } else if (typeof specialist?.answer === 'string') {
    detail = specialist.answer;
  }
...
async function runTutorTurn(input: TutorTurnInput) {
  ...
  return { ...decision, coachMessage: buildTutorMessage(input, decision.action, specialist), ...(specialist ? { specialist } : {}) };
}

function buildServer() {
  const server = new McpServer({ name: 'englishai-mcp-gateway', version: '1.2.0' }, { instructions: 'Use tutor_turn for one-to-one adaptive tutoring and call_capability for specialist MCP work.' });
  ...
  server.registerTool('call_capability', {
    description: 'Execute an approved specialist MCP tool or the stateful tutor orchestrator.',
    inputSchema: z.object({ capability: z.enum(['english-content', 'grammar', 'vocabulary', 'reading', 'writing', 'assessment', 'citation', 'tutor']), tool: z.string().min(1), arguments: z.record(z.string(), z.unknown()).default({}) })
...
createServer(async (req, res) => {
  ...
  if (url.pathname === '/healthz') { res.writeHead(200, { 'content-type': 'application/json' }); res.end(JSON.stringify({ ok: true, service: 'englishai-mcp-gateway', version: '1.2.0', capabilities: Object.keys(capabilityConfig), tutor: true })); return; }
  ...
  const response = await handler.fetch(request); send(res, response);
